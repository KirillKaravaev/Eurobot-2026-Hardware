...
public:
    ...
    virtual int RegWritePos(u8 ID, u16 Position, u16 Time, u16 Speed = 0);// ??????(RegWriteAction??)
    virtual void SyncWritePos(u8 ID[], u8 IDN, u16 Position[], u16 Time[], u16 Speed[]);// ??????
    virtual int PWMMode(u8 ID);// PWM??
    virtual int WritePWM(u8 ID, s16 pwmOut);// PWM????
    ...
    virtual int ReadLoad(int ID);// ????(???0~1000)
